...
/**
 * Vendure's core schema.
 *
 * Generated against an empty database so that a fresh clone can build its schema from
 * migrations alone. Before this existed, the only migration in this directory was the
 * visual-search one, whose foreign keys reference `product` and `asset` — so a new
 * checkout had no way to get a working database, and `runMigrations()` failed on every
 * boot with `relation "public.zone" does not exist`.
 *
 * `product_asset_embedding` is deliberately NOT created here. TypeORM generates it as
 * `embedding text` because the entity has to declare it that way (there is no vector
 * type in TypeORM), which would silently produce a table that pgvector's `<=>` operator
 * cannot use and that the HNSW index cannot be built on. That table belongs to
...
export class BaselineSchema1785980000000 implements MigrationInterface {
   ...
   public async down(queryRunner: QueryRunner): Promise<any> {
        ...
        await queryRunner.query(`DROP INDEX "public"."IDX_0541972555d17d77fe845610b7"`, undefined);
        await queryRunner.query(`DROP INDEX "public"."IDX_3818e31a0350a5c7c00351f0f7"`, undefined);
